...
%%%%%%  SAT Variables:
%me(M,E) means "Moderator M moderates event E"
satVariable( em(E,M) ):-event(E),moderator(M).
%ed(E,D), a més d'Exèrcit d'en Dumbledore, vol dir "l'esdeveniment E és el dia D" (no el del desembarcament de Normandia).
satVariable( ed(E,D) ):-event(E),day(D).
%md(M,D), a més de "Missatge directe" a Twitter, vol dir "el moderador M modera el dia D"
...
eachEventOneDay:-event(E),findall(ed(E,D),day(D),L),exactly(1,L),fail.
eachEventOneDay.

eachDayMaxNumEvents:-day(D), findall(ed(E,D),event(E),L), maxEventsPerDay(M),atMost(M,L), fail.
eachDayMaxNumEvents.

eachEventOneModerator:-eventModerators(E,LM),findall(em(E,M),member(M,LM),L), exactly(1,L),fail.
eachEventOneModerator.

eachModeratorMaxDays:-moderator(M),findall(md(M,D),day(D),L),maxDaysPerModerator(MD),atMost(MD,L),fail.
eachModeratorMaxDays.

escriuCoses:-event(E),day(D),moderator(M),writeClause([-ed(E,D),-em(E,M),md(M,D)]),fail.
escriuCoses.
