...
writeClauses(infinite):- !, N = 6, writeClauses(N),!. % N = 1000 should be replaced with by right N
writeClauses(MaxNumProf):-
    expressOrCD,
    oneCourseOneLectureADay,
    hoursPerWeek,
    ...
    noYearOverlap,
    getProfessors,
    limitProfessors(MaxNumProf),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

professorsWhoTeachDifferentYears(PF):-
	Year(Y1),
	Year(Y2),
	findall(
...
noProfessorOverlap:-
        courseProfessors(C1,P1),
        courseProfessors(C2,P2),
        C1 \= C2,
        member(P,P1),
        member(P,P2),
        day(D),
...
costOfThisSolution(M,Cost):- findall(P,(member(p(P),M)),L),sort(L,L1),length(L1,Cost),!.
