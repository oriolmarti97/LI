% This is a Branch-and-bound solution for a Traveling-Salesman-like problem: find the
...
%
% For this exam: modify the program in order to drop the requirement of starting in
...
main:- N=11, retractall(bestRouteSoFar(_,_)),  assertz(bestRouteSoFar(100000,[])),  % "infinite" distance
       findall(I,between(1,N,I),Cities), select(City, Cities, RemainingCities), tsp( RemainingCities, 0, [City] ).
