...
%% A school wants us to design to weekly schedule of a certain
...
% Year tat have some course being taught between 8 and 14
satVariable(ym(Y)):- year(Y).
...
writeClauses(MaxNumProf):-
    ...
    oneCourseOneLecture,
    hoursPerWeek,
    oneCourseOneProfessor,
    oneCourseOneRoom,
    noOverlapYear,
    oneRoomOneCourse,
    oneProfessorOneCourse,
    findMornignCourses,
    minMornignCourses(MaxNumProf),
    true,!.
...
oneCourseOneProfessor:-
    courseProfessors(C,Ps),
    findall(cp(C,P),member(P,Ps),L),
    exactly(1,L),
    fail.
oneCourseOneProfessor.
...
oneProfessorOneCourse:-
    courseProfessors(C1,R1),
    courseProfessors(C2,R2),
    C1 \= C2,
    member(R,R1),
    member(R,R2),
    day(D),
    hour(H),
    writeClause([-cdh(C1,D,H),-cdh(C2,D,H),-cp(C1,R),-cp(C2,R)]),
    fail.
oneProfessorOneCourse.
...
findMornignCourses:-
    year(Y),
    findall(cdh(C,D,H),(courseYear(C,Y), day(D), between(8,14,H)),L),
    expressOr(ym(Y),L),
    fail.
findMornignCourses.

minMornignCourses(M):-
    findall(ym(T),year(T),L),
    atMost(M,L),
    fail.
minMornignCourses(_).
...
costOfThisSolution(M,Cost):- findall(Y,member(ym(Y),M),L), sort(L,L1), length(L1,Cost), !.
