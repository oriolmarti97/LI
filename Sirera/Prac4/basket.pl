...
sameMatchOnce:-
	difTeams(S,T),
	findall(match-S-T-R,round(R),Lits),
	atMost(1,Lits),
	fail.
sameMatchOnce.
...
tvMatches:-
	round(R),
    findall(match-S-T-R, tvMatch(S,T), LitsH ),
    findall(match-T-S-R, tvMatch(S,T), LitsA ), 
...
noPlayAtHome:-
	findall(home-S-R,away(S,R),Lits),
	atMost(0,Lits),
	fail.
noPlayAtHome.
...
noDoublesFind:-
	noDoubles(ND),
	findall(double-T-R,(team(T),member(R,ND)),D),
	atMost(0,D),
	fail.
noDoublesFind.

noTriplesFind:-
    findall(triple-T-R,(team(T),round(R)),T),
    atMost(0,T),
    fail.
noTriplesFind.
...
defineHome:-
	team(T),
	round(R),
	findall(match-T-S-R,difTeams(T,S),Home),
	expressOr(home-T-R,Home),
	fail.
defineHome.
...
defineDouble:-
	...
	R1 is R - 1,
	team(S),
	expressAnd(doubleHome-S-R,[home-S-R,home-S-R1]),
	expressAnd(doubleAway-S-R,[\+home-S-R,\+home-S-R1]),
	expressOr(double-S-R,[doubleHome-S-R,doubleAway-S-R]),
	fail.
defineDouble.
...
defineTriple:-
	...
	R2 is R - 2,
	team(S),
	expressAnd(tripleHome-S-R,[home-S-R,home-S-R1,home-S-R2]),
	expressAnd(tripleAway-S-R,[\+home-S-R,\+home-S-R1,\+home-S-R2]),
	expressOr(triple-S-R,[tripleHome-S-R,tripleAway-S-R]),
	fail.
defineTriple.
