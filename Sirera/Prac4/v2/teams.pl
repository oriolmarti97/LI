...
createVar:-
    ...
    complexThreshold(N),
    S1 + S2 > N,
    writeClause([-wt(W1,T),-wt(W2,T),c(T)]),
    fail.
createVar.

minComplex(X):-
    findall(c(T),team(T),L),
    atMost(X,L),
    fail.
minComplex(_).
...
costOfThisSolution(M,Cost):- findall(T,member(c(T),M),L), sort(L,L1), length(L1,Cost), !.
