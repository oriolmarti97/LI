...
incompatibleWorkers(W1,W2):- workerScore(W1,S1), workerScore(W2,S2), W1<W2, S is S1+S2, maxScore(Max), S > Max.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 1.- Declare SAT variables to be used
% wt(W,T) means "worker W is in team T"
satVariable( wt(W,T) ):- worker(W), team(T).
satVariable( c(T) ):- team(T).
...
writeClauses(infinite):- !, numTeams(N), writeClauses(N),!. % N = 1000 should be replaced with by right N
writeClauses(MaxComplexTeams):-
    eachTeamCorrectSize,
    eachTeamCompatibleWorkers,
    eachWorkerExactlyOneTeam,
    setComplex,
    atMostKComplex(MaxComplexTeams),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.


eachTeamCorrectSize:- team(T), findall(wt(W,T),worker(W),L), minSize(Min), maxSize(Max), atLeast(Min,L), atMost(Max,L), fail.
eachTeamCorrectSize.

eachTeamCompatibleWorkers:- team(T), incompatibleWorkers(W1,W2), writeClause([-wt(W1,T),-wt(W2,T)]), fail.
eachTeamCompatibleWorkers.

eachWorkerExactlyOneTeam:- worker(W),findall(wt(W,T),team(T),L),exactly(1,L), fail.
eachWorkerExactlyOneTeam.

complexos(W1,W2):-workerScore(W1,S1), workerScore(W2,S2), W1<W2, complexThreshold(S), S3 is S1+S2, S3>=S.

setComplex:- team(T), complexos(W1,W2), writeClause([-wt(W1,T),-wt(W2,T),c(T)]),fail.
setComplex.

atMostKComplex(K):-findall(c(T),team(T),L), atMost(K,L),fail.
atMostKComplex(_).
...
costOfThisSolution(M,Cost):- findall(T, member(c(T),M),L), length(L,Cost),!.
