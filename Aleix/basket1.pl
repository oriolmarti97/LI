...
% The following you already know:
% 
...
cityTeam(CT):- team(CT), between(1,4,CT).
...
eachTeamAproxHalfTheRoundsHomeHalfAway:- team(T), 
    findall(home-T-R, round(R), L), numTeams(N), N1 is N/2, N2 is N/2 - 1, atMost(N1,L), atLeast(N2,L),fail.
eachTeamAproxHalfTheRoundsHomeHalfAway.


eachRoundTwoHomeTwoAwayOf1234:-round(R),
    findall(home-CT-R, cityTeam(CT), L), exactly(2,L),fail.
eachRoundTwoHomeTwoAwayOf1234.
...
costOfThisSolution(M,Cost):-
    between(0,20,I), Cost is 20-I, team(T),
    findall( R, member(double-T-R,M), L ), length(L,Cost), !.
