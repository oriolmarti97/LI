...
heli( [], AccumulatedDistance, [CurrentPoint|RouteSoFar], [] ):- 
	helicopterBasePoint(Dest),
	distanceBetweenTwoPoints(CurrentPoint,Dest,Dist),
	AccumulatedDistance is 
...
heli( PassengersToPickUp, AccumulatedDistance, [CurrentPoint|RouteSoFar], DestinationsOfPassengersInHelicopter ):- 
	select(Passenger,PassengersToPickUp,RemainingPassengers),
	Passenger==CurrentPoint,
	heli(RemainingPassengers,AccumulatedDistance,[CurrentPoint|RouteSoFar],[Passenger|DestinationsOfPassengersInHelicopter]).

% Drop off one of the passengers in the helicopter:
heli( PassengersToPickUp, AccumulatedDistance, [CurrentPoint|RouteSoFar], DestinationsOfPassengersInHelicopter ):-
	select(Dest,DestinationsOfPassengersInHelicopter,RemainingDists),
	Dest==CurrentPoint,
	heli(PassengersToPickUp,AccumulatedDistance,[CurrentPoint|RouteSoFar],RemainingDists). %No té sentit descarregar i fallar (?)
